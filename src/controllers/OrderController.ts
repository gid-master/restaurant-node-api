...
import { OrderModel, OrderSchema, IOrder, IOrderProduct, IOrderReview, ProductSchema } from "@schemas/index";
...
class OrderController extends BaseController {
                        ...
                        $map: {
                          input: {
                            $filter: {
                              ...
                              as: "productsCollection",
                              cond: { $eq: ["$$productsCollection._id", "$$product.productId"] }
                            }
                          },
                          as: "data",
                          in: "$$data.name"
    ...
    public createOrder = async (req:Request, res:Response): Promise<Response> => {
      try {
        ...
        const order: IOrder = {
          ...
          userId: res.get("authenticatedUserId"),
          date: new Date()
        };
    ...
    public reviewOrder = async (req:Request, res:Response): Promise<Response> => {
      try {
        const userId:string = res.get("authenticatedUserId");
        const orderReview:IOrderReview = req.body;

        const result:OrderModel = await OrderSchema.findOne({ _id: orderReview.orderId, userId: userId });
